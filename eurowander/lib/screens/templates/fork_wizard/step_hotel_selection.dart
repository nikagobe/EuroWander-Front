      ...
      builder: (context, provider, _) {
        ...
        return Column(children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (search == null)
                  const Padding(padding: EdgeInsets.all(20), child: Text('No hotel recommendations for this city.', style: TextStyle(color: AppTheme.textSecondary)))
                else if (isLoading)
                  ..._buildSkeletons()
                else ...[
                  // Author's picks
                  ...
                  if (search.fallbackParams != null) ...[
                    const SizedBox(height: 16),
                    const Text('🔍 MORE OPTIONS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppTheme.textSecondary, letterSpacing: 1)),
          ...
          Padding(
            padding: const EdgeInsets.all(16),
            child: Row(children: [
              Expanded(child: OutlinedButton(onPressed: widget.onBack, style: OutlinedButton.styleFrom(foregroundColor: AppTheme.textSecondary, side: BorderSide(color: Colors.grey.withOpacity(0.3)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(vertical: 14)), child: const Text('← Back'))),
              const SizedBox(width: 8),
              TextButton(onPressed: widget.onNext, child: const Text('Skip hotel', style: TextStyle(color: AppTheme.textSecondary))),
              const SizedBox(width: 8),
              Expanded(child: ElevatedButton(onPressed: widget.onNext, style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryColor, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(vertical: 14)), child: const Text('Next city →'))),
            ]),
          ),
        ]);
      },
